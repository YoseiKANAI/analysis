...
def main():
    ...
    for ID in range(g.subnum):
        ...
        sheet_name = "mean_actibity_level_sub%d" %(ID+1)
        result = create_mean_actibity_level_data(filename_list, ID)
...
"""
def create_mean_actibity_level_data(filename_list, ID):
    # 格納するDataFrameを作成
    ...
    # ファイルごとに計算
        for f in task_list:
            ...
            result = cal_mean_actibity_level(df, cnt, result)
            cnt += 1
...
"""
mean_actibity_levelの計算を行う関数
"""
def cal_mean_actibity_level(df, cnt, result):
    mean_actibity_level = df.mean(axis = 0, skipna=True)
    result.iloc[cnt, :] = mean_actibity_level
    return result

"""
被験者ごとの平均と分散を算出し，格納
"""
def cal_mean_std(mean_actibity_level, columns_list):
    mean_actibity_level = pd.concat([mean_actibity_level, pd.DataFrame(index=[""])])
    # タスクごとの平均と分散を算出
    for i in range(task_num):
        # 空白行を挿入
        mean_actibity_level = pd.concat([mean_actibity_level, pd.DataFrame(index=[g.task[i]])])
        
        # mean_actibity_levelからタスクごとのデータを取り出し
        df = np.array(mean_actibity_level.iloc[i*5:(i+1)*5])
        ...
        # mean_actibity_levelに格納
        mean_actibity_level = pd.concat([mean_actibity_level, mean])
        mean_actibity_level = pd.concat([mean_actibity_level, std])
    return mean_actibity_level
...
def output_preparation():
    # ルートフォルダのパスを指定
    output_dir = "D:/User/kanai/Data/%s/result_EMG/mean_actibity_level" %(g.datafile)
    output_dir_plot = "D:/User/kanai/Data/%s/result_EMG/mean_actibity_level/plot" %(g.datafile)
...
def sub_gragh_plot(result, output_dir_plot, ID):
    ...
    ax.set_ylabel("EMG mean actibity level [%MVC]")
...
def summary_gragh_plot(mean, std, output_dir, muscle):
    ...
    for m in range(len(muscle)):
        ...
        for t in range(len(task_list)):
            slide = t*0.17
            err = [std_group[:, t, m]]
            ax.bar(sub_num+slide, mean_group[:, t, m], width=0.15
