...
def main():
    summary_mean = np.full(((g.subnum), task_num, g.muscle_num), np.nan)
    summary_std = np.full(((g.subnum), task_num, g.muscle_num), np.nan)
    output_name, output_dir_plot = output_preparation()
    for ID in range(g.subnum):
        filename_list = input_preparation(ID)
        sheet_name = "iEMG_sub%d" %(ID+1)
        result = create_iEMG_data(filename_list, ID)
...
#        sub_gragh_plot(result.iloc[69:84, :], output_dir_plot, ID)
        ...
        for t in range(task_num):
            summary_mean[ID, t, :] = result.iloc[t*3+27,:]
            summary_std[ID, t, :] = result.iloc[t*3+28,:]
...
"""
def create_iEMG_data(filename_list, ID):
    # 格納するDataFrameを作成
    ...
    # ファイルごとに計算
        for f in task_list:
            ...
            if t=="D2" and attempt_num == 2 and ID ==1:
                result = pd.concat([result, pd.DataFrame(index=[index_name])])
                continue
...
def output_preparation():
    # ルートフォルダのパスを指定
    output_dir = "D:/User/kanai/Data/%s/result_EMG/iEMG" %(g.datafile)
    output_dir_plot = "D:/User/kanai/Data/%s/result_EMG/iEMG/plot" %(g.datafile)
    ...
    for m in range(len(muscle)):
        ...
        for t in range(len(task_list)):
            ...
            ax.bar(sub_num+slide, mean_group[:, t, m], width=0.12
                , yerr=err, capsize=3, label = task_list[t])
        ax.legend(loc = "upper right", fontsize ="large", ncol=task_num, frameon=False, handlelength = 0.7, columnspacing = 1)
        ax.tick_params(direction="in")
