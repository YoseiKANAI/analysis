...
for ID in range(g.subnum):
    ...
    output_dir = "D:/User/kanai/Data/%s/result_FFT/sub%d/segment" %(g.datafile, ID+1)
    ...
    for dirpath, dirnames, filenames in os.walk(root_dir):
        ...
        for filename in filenames:
            ...
            if g.sub_domi[ID] == 0:
                list = ["finger X", "finger Y", "finger Z","RWRA X", "RWRA Y", "RELB X", "RELB Y"]
                for i in list:
                    seg[i] = df[i]
            elif g.sub_domi[ID] == 1:
                list = ["finger X", "finger Y","finger Z", "LWRA X", "LWRA Y", "LELB X", "LELB Y"]
                for i in list:
                    seg[i] = df[i]
            ...
            for i in list:
                # welch法でスペクトル解析
                freqs, psd = welch(seg[i], fs = f_s, nperseg=2048)           
