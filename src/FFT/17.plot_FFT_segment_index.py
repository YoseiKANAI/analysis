...
for ID in range(g.subnum):
    subID = "sub%d" %(ID+1)
    if g.domi_arm[ID] == 0:
        seg =  ["finger X", "finger Y", "finger Z","RWRA X", "RWRA Y", "RELB X", "RELB Y"]
    elif g.domi_arm[ID] == 1:
        seg =  ["finger X", "finger Y","finger Z", "LWRA X", "LWRA Y", "LELB X", "LELB Y"]

    # ルートフォルダのパスを指定
    root_dir = "D:/User/kanai/Data/%s/result_FFT/sub%d/segment" %(g.datafile, ID+1)
    
    output_dir = os.path.join( root_dir, "index")
    # 出力先フォルダを作成
    ...
    for i in seg:
        ...
        for t in range(3):
            ...
            ax.bar(index_num+slide, mean.iloc[t, :], width=0.2, yerr=err, capsize=3, label = task[t], color = color[t])
