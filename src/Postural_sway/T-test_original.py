# -*- coding: utf-8 -*-
"""
Created on Sat Jun 18 17:17:51 2022
...
def run():
    """ 要変更 """
    taskList = ['NC', 'VLTC', 'Phone-VLTC', 'Phone Swing', 'Phone Hold']
    """ 要変更終了 """

    kindOfTtest = decide_kind_of_Ttest()
    path = './result/*.csv'
    results = []  # resultまとめファイル(dataframeのlist(3次元)) 例 results[0]=DataFrame, results[1]=DataFrame
    subList = []  # 被験者列(list)
    ...
    (subNum, trialNum) = preparation(path, results)
    ...
    totalDxyDf = loop(subNum, taskNum, trialNum, totalDxy, results, totalDxyColumn, normalizeTotalDxy, subList,
                      trialList, df1, taskList, wholeAverageBeforeNormalization, wholeAverageAfterNormalization,
                      wholeSDBeforeNormalization, wholeSDAfterNormalization, kindOfTtest)
    rmsXDf = loop(subNum, taskNum, trialNum, rmsX, results, rmsXColumn, normalizeRmsX, subList, trialList, df1,
                  taskList, wholeAverageBeforeNormalization, wholeAverageAfterNormalization, wholeSDBeforeNormalization,
                  wholeSDAfterNormalization, kindOfTtest)
    rmsYDf = loop(subNum, taskNum, trialNum, rmsY, results, rmsYColumn, normalizeRmsY, subList, trialList, df1,
                  taskList, wholeAverageBeforeNormalization, wholeAverageAfterNormalization, wholeSDBeforeNormalization,
                  wholeSDAfterNormalization, kindOfTtest)
    ampWidthXDf = loop(subNum, taskNum, trialNum, ampWidthX, results, ampWidthXColumn, normalizeAmpWidthX, subList,
                       trialList, df1, taskList, wholeAverageBeforeNormalization, wholeAverageAfterNormalization,
                       wholeSDBeforeNormalization, wholeSDAfterNormalization, kindOfTtest)
    ampWidthYDf = loop(subNum, taskNum, trialNum, ampWidthY, results, ampWidthYColumn, normalizeAmpWidthY, subList,
                       trialList, df1, taskList, wholeAverageBeforeNormalization, wholeAverageAfterNormalization,
                       wholeSDBeforeNormalization, wholeSDAfterNormalization, kindOfTtest)
    rectangleAreaDf = loop(subNum, taskNum, trialNum, rectangleArea, results, rectangleAreaColumn,
                           normalizeRectangleArea, subList, trialList, df1, taskList, wholeAverageBeforeNormalization,
                           wholeAverageAfterNormalization, wholeSDBeforeNormalization, wholeSDAfterNormalization,
                           kindOfTtest)
    rmsAreaDf = loop(subNum, taskNum, trialNum, rmsArea, results, rmsAreaColumn, normalizeRmsArea, subList, trialList,
                     df1, taskList, wholeAverageBeforeNormalization, wholeAverageAfterNormalization,
                     wholeSDBeforeNormalization, wholeSDAfterNormalization, kindOfTtest)
    sdAreaDf = loop(subNum, taskNum, trialNum, sdArea, results, sdAreaColumn, normalizeSdArea, subList, trialList, df1,
                    taskList, wholeAverageBeforeNormalization, wholeAverageAfterNormalization,
                    wholeSDBeforeNormalization, wholeSDAfterNormalization, kindOfTtest)
    wholeBeforeNormalization = create_whole(taskNum, taskList, wholeAverageBeforeNormalization,
                                            wholeSDBeforeNormalization)
    wholeAfterNormalization = create_whole(taskNum, taskList, wholeAverageAfterNormalization, wholeSDAfterNormalization)

    outputPath = './result/T test_'
    ...
    output_file(outputPath, wholeBeforeNormalization, wholeAfterNormalization, totalDxyDf, rmsXDf, rmsYDf, ampWidthXDf,
                ampWidthYDf, rectangleAreaDf, rmsAreaDf, sdAreaDf, '正規化前全体', '正規化後全体', '総軌跡長', 'SDx',
                'SDy', '振幅幅x', '振幅幅y', '矩形面積', '実効値面積', '標準偏差面積')
...
def preparation(path, results):
    file = glob.glob(path)  # ディレクトリ下のファイル取得
...
def average_NC(subNum, trialNum, results, column):
    ...
    for i in range(subNum):
        for j in range(trialNum):
            ave[i] += float(results[i].iat[j, column])
        ave[i] /= trialNum
...
def individual(subNum, taskNum, trialNum, value):
    ...
    for k in range(taskNum):
        for i in range(subNum):
            for j in range(trialNum):
                tmp[i] += value[k][j + i * trialNum]
            tmp[i] /= trialNum
            tmpSD.append(st.stdev(value[k][i * trialNum:i * trialNum + trialNum]))
...
# valueのdataframe作成
def create_dataframe(value, normalization, subList, trialList, subNum, trialNum, taskNum):
    ...
    # リスト結合
    value = subList + trialList + value + emptyList + subList + trialList + normalization
...
def create_variousValues(value, normalization, taskNum, taskList, wholeAverageBeforeNormalization,
                         wholeAverageAfterNormalization, wholeSDBeforeNormalization, wholeSDAfterNormalization):
    variousValues = []
...
def p_value(value, taskNum, kindOfTtest):
    ...
    for i in range(taskNum):
        for j in range(i + 1, taskNum):
            if kindOfTtest == 1:
                # 対応のあるt検定
                T, P = stats.ttest_rel(value[i], value[j])
                pValue.append(P)
            elif kindOfTtest == 2:
                # welchのt検定
...
def create_pList(value, normalization, taskNum, taskList, kindOfTtest):
    ...
    # 結合
    pList = [task1] + [task2] + [pValue] + [pRank] + holm + [emptyList] + [task1] + [task2] + [pValue_normalization] + [
        pRank_normalization] + holm_normalization
...
def create_individualFrame(value, normalization, subNum, taskNum, trialNum, taskList):
    ...
    resultNormalization, SDNormalization = individual(subNum, taskNum, trialNum, normalization)
    ...
    # 結果の行を結合
    result = [emptyList] + [subList] + resultValue + [emptyList] + [emptyList] + [subList] + resultNormalization
    SD = [emptyList] + [subList] + SDValue + [emptyList] + [emptyList] + [subList] + SDNormalization
...
# ファイル出力
def output_file(path, df1, df2, df3, df4, df5, df6, df7, df8, df9, df10, sheetName1, sheetName2, sheetName3, sheetName4,
                sheetName5, sheetName6, sheetName7, sheetName8, sheetName9, sheetName10):
...
# 繰り返し部分
def loop(subNum, taskNum, trialNum, value, results, column, normalization, subList, trialList, df1, taskList,
         wholeAverageBeforeNormalization, wholeAverageAfterNormalization, wholeSDBeforeNormalization,
         wholeSDAfterNormalization, kindOfTtest):
    # 下処理
    ...
    # valueのdataframe作成
    df2 = create_dataframe(value, normalization, subList, trialList, subNum, trialNum, taskNum)

    # variousValuesのdataframeの作成
    df3 = create_variousValues(value, normalization, taskNum, taskList, wholeAverageBeforeNormalization,
                               wholeAverageAfterNormalization, wholeSDBeforeNormalization, wholeSDAfterNormalization)
    ...
    # individualのdataframeの作成
    df5 = create_individualFrame(value, normalization, subNum, taskNum, trialNum, taskList)
...
def create_whole(taskNum, taskList, wholeAverage, wholeSD):
    ...
    # 全結合
    whole = [firstLine] + [secondLine] + [totalDxy] + [rmsX] + [rmsY] + [ampWidthX] + [ampWidthY] + [rectangleArea] + [
        rmsArea] + [sdArea]
