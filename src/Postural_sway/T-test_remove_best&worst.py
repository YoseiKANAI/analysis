...
def run():
    ...
    outputPath += "_sub" + str(subNum) + "_remove_bestworst.xlsx"
    output_file(
        outputPath,
...
def preparation(path, results):
    ...
    for i in range(subNum):
        ...
        for k in range(tasknum):
            max_id = (tmpFile.iloc[k*(tasknum+1)+1 : (k+1)*(tasknum+1) , 18]).astype(float).idxmax()
            min_id = (tmpFile.iloc[k*(tasknum+1)+1 : (k+1)*(tasknum+1) , 18]).astype(float).idxmin()
